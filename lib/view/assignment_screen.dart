...
class _AssignmentScreenState extends State<AssignmentScreen> {
  ...
  Future<void> uploadFile(PlatformFile file) async {
    final filename =
        '${file.name.split('.').first}_${status.userId}_${status.chapterId}_${DateTime.now().millisecondsSinceEpoch}.${file.extension}';
  ...
  void updateProgressAssignment() {
    if (status.assignmentDone &&
        status.isCompleted &&
        !showDialogAssignmentOnce) {
      showDialogAssignmentOnce = true;
      showCompletionDialog(context,
          "Yeay kamu berhasil menyelesaikan Chapter ini, Ayo lanjutkan pelajari chapter yang lain");
    }
  }
    ...
    FileDownloader.downloadFile(
      ...
      onDownloadCompleted: (filePath) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Download Complete, saved in $filePath",
              style: TextStyle(fontFamily: 'DIN_Next_Rounded'),
            ),
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  onPressed: () async {
                                    ...
                                    Duration difference = status.timeStarted
                                        .difference(status.timeFinished);
                                    if (!status.assignmentDone && !complete) {
                                      user?.points = user!.points! +
                                          calculatePoint(difference.inMinutes);
                                    }

                                    if (widget.level == uc.currentChapter) {
                                      uc.currentChapter++;
                                      uc.progress = (((uc.currentChapter - 1) /
                                                  chLength) *
                                              100)
                                          .toInt();
                                    }
                                  ...
                                  icon: Icon(LineAwesomeIcons.paper_plane_solid,
                                      color: Colors.white),
                                  label: Text(
                                    'Submit',
                                    style: TextStyle(
              ...
              children: [
                ...
                TextButton.icon(
                  ...
                  label: Text('Ganti File',
                      style: TextStyle(
                          color: Colors.grey, fontFamily: 'DIN_Next_Rounded')),
                ),
              ],
            ),
          ],
      ...
      child: SizedBox(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              Image(
                image: AssetImage(file.extension == 'pdf'
                    ? 'lib/assets/iconpdf.png'
                    : file.extension == 'jpg'
                        ? 'lib/assets/iconjpg.png'
                        : ''),
