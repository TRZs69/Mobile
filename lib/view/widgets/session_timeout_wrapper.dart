...
class _SessionTimeoutWrapperState extends State<SessionTimeoutWrapper> with WidgetsBindingObserver {
  Timer? _timer;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Dipanggil saat aplikasi pindah ke background atau kembali ke foreground
  ...
  Future<void> _checkSessionOnStart() async {
    ...
    if (lastActivityStr != null) {
      ...
      if (difference >= widget.timeout) {
        // Jika sudah melewati batas waktu saat aplikasi ditutup
        _handleTimeout();
        return;
      }
    }
    
    // Jika masih dalam batas waktu, jalankan timer seperti biasa
    _startTimer();
  }
  ...
  void _startTimer() {
    ...
    _recordActivity(); // Simpan timestamp ke disk
  }
  ...
  void _handleTimeout() async {
    ...
    // Hapus data timestamp aktivitas agar tidak logout berulang
    final prefs = await SharedPreferences.getInstance();
